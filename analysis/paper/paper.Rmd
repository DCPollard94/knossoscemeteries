...
```{r burialestimate, fig.cap="Relationship between known interments, counts of pithoi, and counts of pithoi, amphorae and kraters. The straight line represents the mean estimate based on these three values."}
...
#The data from the tomb database are added, and 'attached' so that the variables therein need no longer be referred to in their long format (i.e. 'tombs$total_pottery' becomes simply 'total_pottery')
...
colnames(aorist_all_tombs)<-c("aoristic_density")
dates<-seq(from = 610, to = 1100, by = 10)
dates<-rev(dates)
...
ggplot(aorist_all_tombs,aes(decade,aoristic_density, fill="Aoristic Sum"))+
...
colnames(aorist_KNC_pithoi)<-c("aoristic_density")
...
colnames(aorist_FOR_pithoi)<-c("aoristic_density")
...
colnames(aorist_all_pithoi)<-c("aoristic_density")
...
aorist_pithoi_plot<-ggplot(NULL)+
  geom_col(data=aorist_all_pithoi,aes(decade,aoristic_density,fill=" All tombs  "))+
    geom_col(data=aorist_KNC_pithoi,aes(decade,aoristic_density,fill=" KNC  "))+
      geom_col(data=aorist_FOR_pithoi,aes(decade,aoristic_density, fill=" Fortetsa  "))+
        theme_bw()+theme(legend.title=element_blank(),
          text=element_text(family="Garamond", size=12))+
...
colnames(aorist_KNC)<-c("aoristic_density")
...
colnames(aorist_FOR)<-c("aoristic_density")
...
aorist_pots_plot<-ggplot(NULL)+
  geom_col(data=aorist_all_tombs,aes(decade,aoristic_density,fill=" All tombs  "))+
    geom_col(data=aorist_KNC,aes(decade,aoristic_density,fill=" KNC  "))+
      geom_col(data=aorist_FOR,aes(decade,aoristic_density, fill=" Fortetsa  "))+
        theme_bw()+theme(legend.title=element_blank(),
          text=element_text(family="Garamond", size=12))+
...
ggplot(NULL)+geom_col(data=aorist_all_tombs,aes(decade,aoristic_density,fill="All tombs"))+
  geom_col(data=aorist_KNC,aes(decade,aoristic_density,fill="KNC"))+
  geom_col(data=aorist_FOR,aes(decade,aoristic_density, fill="Fortetsa"))+
...
aorist_pot_medians$aorist_sums<-aorist_all_tombs$aoristic_density
aorist_pot_medians$aorist_median_adjust<-aorist_pot_medians$aorist_sums/aorist_pot_medians$aoristic_median
View(aorist_pot_medians)
...
colnames(aorist_pithoi)<-c("aoristic_density")
...
colnames(aorist_all_urns)<-c("aoristic_density")
...
ggplot(NULL)+geom_col(data=aorist_all_urns,aes(decade,aoristic_density,fill="All urns"))+
  geom_col(data=aorist_pithoi,aes(decade,aoristic_density,fill="Pithoi"))+
      xlim(1100,630)+theme_minimal()+
        theme(legend.title=element_blank(),
          text=element_text(family="Garamond", size=12))+
              labs(x="Date (Years BC)", y="Aoristic Density")+
...
aorist_by_tomb$aorist_by_tomb<-aorist_by_tomb$aoristic_density/aorist_by_tomb$no_tombs
...
colnames(aorist_tombs_9th)<-c("aoristic_density")
...
aorist_by_tomb_9th$aorist_by_tomb<-aorist_by_tomb_9th$aoristic_density/aorist_by_tomb_9th$no_tombs
...
colnames(aorist_tombs_top)<-c("aoristic_density")
...
aorist_by_tomb_top$aorist_by_tomb<-aorist_by_tomb_top$aoristic_density/aorist_by_tomb_top$no_tombs
