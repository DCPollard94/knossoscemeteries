...
For analysis at the level of individual finds, however, pithoi alone were taken as the best proxy, as they correlated most closely with the mean estimate, producing a pearson correlation coefficient of `r cor_burial_proxy` (Figure \@ref(fig:burialestimate)). For the bulk of the analyses, any tombs lacking identified remains or a single vessel suitable for use as an urn were not considered, thus excluding the most heavily damaged and looted tombs.


```{r aorist_all_cems}

aorist_all_tombs <- colSums(main_tombs[157:206])
...
ggplot(aorist_all_tombs,aes(decade,aoristic_density))+
  geom_col()+xlim(1100,630)+theme_minimal()+
        theme(legend.title=element_blank(),
          text=element_text(family="Garamond", size=12))+
...
ggplot(main_tombs,aes(total_finds))+geom_histogram()+xlim(0,200)
...
ggplot(aorist_melt,aes(variable,value, fill=period))+geom_boxplot(varwidth=TRUE)+theme_bw()+
      theme(legend.title=element_blank(),
        text=element_text(family="Garamond", size=12),
...
for(i in colnames(aorist_tomb_values[2:51])) {
  tombs_gini[[paste0(i)]] <- ineq(aorist_tomb_vals_only[[i]],type="Gini")
}
...
ggplot() + 
  geom_rect(data = rects, aes(xmin = xstart, xmax = xend, ymin = -Inf, ymax = Inf, fill = col), alpha = 0.4) +
     geom_line(data = tombs_gini, aes(decade,gini_coefficient))+xlim(1100,600)+ylim(0.5,0.8)+
      theme_bw()+theme(legend.title=element_blank(),
          text=element_text(family="Garamond", size=12))+
              labs(x="Date (Years BC)", y="Gini Coefficient")+
                 scale_fill_brewer(palette = "Set3")+ scale_x_reverse(expand = c(0, 0),limits=c(1100,600))+
...
no_tombs<-colSums(main_tombs[157:206]!=0)
...
names(main_tombs)
tombs_in_use<-colSums(main_tombs[5:14]!="")
...
tombs_ranked<-main_tombs[with(main_tombs, order(-main_tombs$total_pottery)), ]
