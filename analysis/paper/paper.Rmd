...
ggplot(aorist_melt,aes(variable,value,fill=period))+geom_boxplot(aes(group = cut_width(variable, 10)))+theme_bw()+
      theme(legend.title=element_blank(),
        text=element_text(family="Garamond", size=12),
...
aorist_pithoi<-colSums(pithoi[25:74])
...
aorist_all_urns<-colSums(all_urns[25:74])
...
```{r top_tombs, fig.dim = c(7, 5), dpi=300, fig.align='center',}
...
ggplot(aorist_all_tomb_melt,aes(variable, value))+geom_col()+theme(legend.position = "none")+facet_wrap(~tomb)+
  geom_col()+theme_bw()+theme(legend.title=element_blank(),
      text=element_text(family="Garamond", size=12))+
          labs(x="Date (Years BC)", y="Aoristic Sum")+
              scale_fill_brewer(palette = "Set3")+ 
                theme(legend.position="none")+
                  coord_cartesian(ylim=c(0,30),xlim=c(1100,610))+scale_y_continuous(expand=c(0,0))+
